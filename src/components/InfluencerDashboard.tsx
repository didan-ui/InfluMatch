              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between px-3 md:px-4 py-2 md:py-3 rounded-xl md:rounded-2xl text-xs font-bold transition-all cursor-pointer shrink-0 whitespace-nowrap gap-2 ${
                    isActive 
                      ? "bg-brand-sage/55 border-b-2 md:border-b-0 md:border-l-4 border-brand-sage-dark text-brand-text shadow-sm" 
                      : "text-brand-text-soft hover:bg-brand-bg/50 hover:text-brand-text"
                  }`}
                >
                  <div className="flex items-center gap-2 md:gap-3">
                    <Icon className={`w-3.5 h-3.5 md:w-4 md:h-4 shrink-0 ${isActive ? 'text-brand-sage-dark' : 'text-brand-text-light'}`} />
                    <span>{item.label}</span>
                  </div>
                  {item.badge !== undefined && item.badge > 0 && (
                    <span className="bg-brand-blush-dark/95 text-brand-white text-[9px] font-mono font-bold px-1.5 md:px-2 py-0.5 rounded-full select-none ml-1.5">
                      {item.badge}
                    </span>
                  )}
                </button>
